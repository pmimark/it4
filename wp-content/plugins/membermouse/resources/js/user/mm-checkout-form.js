...
var MM_CheckoutView = MM_Core.extend({
    ...
    checkoutx: function(serviceToken, doSubmit) {
        ...
        if (isFree == true) {
            ...
            return mmjs.checkout(doSubmit, "mm_field_billing_address,mm_field_billing_city,mm_field_billing_state,mm_field_billing_zip,mm_field_billing_country," + "mm_field_shipping_address,mm_field_shipping_city,mm_field_shipping_state,mm_field_shipping_zip,mm_field_shipping_country," + "mm_field_cc_number,mm_field_cc_cvv");
        }
    },
    ...
    countryChangeHandler: function(type) {
	    ...
	    var values = {
	      mm_action: "loadStates",
			country: jQuery("#mm_field_" + type + "_country").val(),
			type: type,
	      referer: jQuery("input[name='_wp_http_referer']").val(),
	    };
	    values[mm_nonce_name_checkout_form] = jQuery("input[name='"+mm_nonce_name_checkout_form+"']").val();
	    ajax.send(values, false, 'mmjs', 'countryChangeHandlerCallback');
		 return false;
    },
	 countryChangeHandlerCallback: function(data) {
		 if(data.type == "error") {
			 alert(data.message);
		 } else {
			jQuery("#mm_field_" + data.data.type + "_state_dd").empty().append(data.data.html);
		 	jQuery("#mm_field_" + data.data.type + "_state").val("");
			jQuery("#mm_field_" + data.data.type + "_state").hide();
		 }
		 return false;
	 },
    stateChangeHandler: function(type) {
      var iso = jQuery("#mm_field_" + type + "_country").val();
      if(jQuery("#mm_field_" + type + "_state_dd").val() == "-1") {
        jQuery("#mm_field_" + type + "_state").show().focus();
      } else {
        jQuery("#mm_field_" + type + "_state").val(jQuery("#mm_field_" + type + "_state_dd").val());
    ...
    callPreCheckoutFunctions: function()
    {
    	...
    	if ((serviceToken.length == 0) || (serviceToken == ""))
    	{
	        ...
	        {
	    		callbackInfo = this.preCheckoutCallbacks[i];
