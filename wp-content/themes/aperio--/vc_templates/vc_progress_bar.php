...
---------------------------------------------------------- */
 ...
 foreach($graph_lines_data as $line) {
  ...
  if( $bar_color != ''){ $style1 = "background-color:{$bar_color}";} 
  if( $fbar_color != ''){ $style2 = "background-color:{$fbar_color}";} 
  
  $output .= '<div class="progress '.$bar_options.'" style="'.$style1.'"><div class="bar" style="'.$style2.'" data-percentage-value="'.($percentage_value).'" data-value="'.$line['value'].'"></div></div>';
  $output .= '</div>';
  }
