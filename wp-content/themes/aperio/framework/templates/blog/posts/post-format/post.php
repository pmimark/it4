...
<div id="post-<?php the_ID(); ?>" <?php post_class(' post-standard post-entries bg-style-'.$brad_data['grid_blog_style'].' maxwidth-'.$brad_data['blog_maxwidth'].' '.$ex_class.' clearfix '); ?>>
  ...
  <!-- post content -->
   <?php global $show_readmore; $show_readmore = 0 ;?>
   
  <div class="post-content">
    <?php if(empty( $post->post_excerpt ) && $brad_data['check_excerpts'] != true ) { ?>
    <?php the_content('<span class="button  button_alternateprimary">'. __("Read More", 'brad') . '</span>'); ?>
    <?php } else { ?>
    <?php echo get_the_excerpt(); ?>
    <?php } ?>
  </div>
  
  <?php if($brad_data['check_author'] || (( $brad_data['check_excerpts'] == true && $brad_data['check_readmore']) || $show_readmore == 1 ) || (comments_open() && $brad_data['check_blog_comments']) || $brad_data['check_postlove']  ):?>
  <div class="post-bottom">
    <?php if( ($brad_data['check_excerpts'] == true && $brad_data['check_readmore']) || $show_readmore == 1 ) { echo '<a href="'. get_the_permalink() .'" class="button button_alternateprimary">'. __('Read More','brad').'</a>'; } ?>
