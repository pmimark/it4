...
class WPBakeryShortCode_sc_button extends WPBakeryShortCode {
	...
	protected function content( $atts, $content = null ) {
		...
		if ( is_user_logged_in() ) {
			...
			$a_href = ( $loggedin_link['url'] == '[sc_logout]') ? wp_logout_url() : $loggedin_link['url'];
			$a_title = $loggedin_link['title'];
			$a_target = $loggedin_link['target'];
			$btn_title = $loggedin_text;
		} else {
			...
			$btn_title = $loggedout_text;
		}

		if($title != 'Text on the button'){
			$title = $title;
		}else{
			$title = $btn_title;
		}
...
if(function_exists("vc_map"))
{
	...
	'params' => array(
		...
		array(
			'type' => 'textfield',
			'heading' => __( 'Logged-in button text', 'js_composer' ),
			'holder' => 'button',
			'class' => 'vc_btn',
			'param_name' => 'loggedin_text',
		...
		array(
			'type' => 'textfield',
			'heading' => __( 'Logged-out button text', 'js_composer' ),
			'holder' => 'button',
			'class' => 'vc_btn',
			'param_name' => 'loggedout_text',
			'value' => __( 'Text on the button', 'js_composer' ),
			'description' => __( 'Text on the button.', 'js_composer' )
		),
		array(
